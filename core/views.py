...
@login_required
@require_POST
def order_container_set_tracker_checked_view(request, container_id: int):
    """Persist the manual Updated checkbox for one order tracker row."""
    user = request.user
    if user.is_superuser:
        container = get_object_or_404(OrderContainer, pk=container_id, is_archived=False)
    else:
        company = get_object_or_404(Company, owner=user)
    ...
    raw_checked = (request.POST.get("checked") or "").strip().lower()
    checked = raw_checked in {"1", "true", "yes", "on"}
    if container.tracker_checked != checked:
        container.tracker_checked = checked
        # Do not touch updated_at: checking this box should not reorder the tracker.
        container.save(update_fields=["tracker_checked"])

    return JsonResponse({"ok": True, "checked": container.tracker_checked})


@login_required
@require_POST
def order_tracker_reset_checks_view(request):
    """Clear all manual tracker checkboxes for the current company."""
    user = request.user
    containers = OrderContainer.objects.filter(is_archived=False)
    if not user.is_superuser:
        company = get_object_or_404(Company, owner=user)
        containers = containers.filter(company=company)

    reset_count = containers.filter(tracker_checked=True).update(tracker_checked=False)
    messages.success(request, f"Reset {reset_count} order tracker checkbox{'es' if reset_count != 1 else ''}.")
    return redirect(request.META.get("HTTP_REFERER") or "order_tracker")
